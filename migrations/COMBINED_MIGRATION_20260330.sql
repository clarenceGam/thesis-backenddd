...
-- Date: March 30, 2026
-- Purpose: Consolidate all recent migrations into one comprehensive file
-- ============================================================================
-- This file combines the following migrations:
-- 1. Middle name fields
-- 2. Bar owner registration fields v2
-- 3. Bar reservation time limits
-- 4. Business registration email verification
-- 5. Reservation grace period and no-show
-- 6. Table management dynamic setup
-- 7. Packages tables
-- 8. Staff types and inventory units
-- 9. Inventory requests
-- 10. Event type for bar events
-- 11. Payroll settings
-- 12. Permit expiry tracking
-- ============================================================================
...
ALTER TABLE `bars` 
ADD COLUMN IF NOT EXISTS `staff_types` JSON DEFAULT NULL COMMENT 'Array of staff types present at the bar' AFTER `bar_types`;

-- 2.3 Add permit expiry tracking to bars table
ALTER TABLE `bars` 
ADD COLUMN IF NOT EXISTS `permit_expiry_date` DATE DEFAULT NULL COMMENT 'Business permit expiry date',
ADD COLUMN IF NOT EXISTS `permit_status` ENUM('valid', 'expiring_soon', 'expired') DEFAULT 'valid' COMMENT 'Permit status: valid, expiring_soon (30 days), expired',
ADD COLUMN IF NOT EXISTS `permit_expiry_notified_at` TIMESTAMP NULL DEFAULT NULL COMMENT 'Last time expiry notification was sent',
ADD COLUMN IF NOT EXISTS `permit_expired_flagged_at` TIMESTAMP NULL DEFAULT NULL COMMENT 'When permit was flagged as expired';

-- Add indexes for efficient querying
CREATE INDEX IF NOT EXISTS `idx_permit_expiry_date` ON `bars` (`permit_expiry_date`);
CREATE INDEX IF NOT EXISTS `idx_permit_status` ON `bars` (`permit_status`);
...
ADD COLUMN IF NOT EXISTS `event_type` VARCHAR(100) DEFAULT NULL COMMENT 'Event type: Stand-up Comedy, Open Mic, Live Band, DJ Night, Ladies Night, Custom' AFTER `title`;
...
CREATE INDEX IF NOT EXISTS `idx_payroll_settings_bar` ON `payroll_settings` (`bar_id`);
...
-- This file can be run on a fresh database or existing database.
-- All ALTER TABLE statements use IF NOT EXISTS or ADD COLUMN IF NOT EXISTS
